...
ระบบมี 2 มุมมองหลัก: **ผู้ซื้อ (Member)** กับ **ผู้ดูแลระบบ (Admin)**
ทั้งสองล็อกอินผ่านหน้าเดียวกัน แต่ปลายทางต่างกันตามสิทธิ์ (`role`)

### บัญชีตัวอย่าง (Demo Accounts)

ระบบมีบัญชีตัวอย่าง 2 บัญชีให้ทดสอบทันทีหลัง import ฐานข้อมูล

| มุมมอง | Email | Password | สิทธิ์ |
|---------|--------|-----------|--------|
| **ผู้ซื้อ (Member)** | `demo@gadgetzone.com` | `Demo@1234` | ซื้อสินค้า, ดูประวัติคำสั่งซื้อ, แก้โปรไฟล์ |
| **ผู้ดูแล (Super Admin)** | `admin@gadgetzone.com` | `Admin@1234` | จัดการสินค้า / คำสั่งซื้อ / ผู้ใช้ / ตั้งค่าทั้งระบบ |

> รหัสผ่านถูก hash ด้วย bcrypt (`password_hash()`) เก็บในคอลัมน์ `password` ของตาราง `users`

---

### มุมมองที่ 1: ผู้ซื้อ (Member)

#### ขั้นตอนเข้าสู่ระบบ

1. เปิดหน้า Home ของเว็บ
   - XAMPP: http://localhost/gadgetzone/
   - CAMPP: http://127.0.0.1:8080/gadgetzone/
2. คลิกไอคอน **บุคคล (👤)** มุมขวาบนของ navbar → ไปหน้า Login
3. กรอก:
   - Email: `demo@gadgetzone.com`
   - Password: `Demo@1234`
4. คลิก **Log In** → ระบบพาเข้าหน้า **My Account**

#### สิ่งที่ Member ทำได้

- เลือกสินค้า → กด **Add to Cart**
- ดูตะกร้า → ปรับจำนวน → กด **Proceed to Checkout**
- กรอกที่อยู่จัดส่ง + เลือกวิธีชำระเงิน → กด **Place Order**
- เข้า **My Account** ดู:
  - Dashboard (สถิติคำสั่งซื้อ + ยอดที่ใช้ไป)
  - My Orders (ประวัติคำสั่งซื้อทั้งหมด)
  - Profile (แก้ไขชื่อ ที่อยู่ เบอร์โทร + อัปโหลดรูป)
  - Change Password (เปลี่ยนรหัสผ่าน)

#### สิ่งที่ Member ทำ**ไม่ได้**

- เข้าหน้า `/admin/*` (ระบบ redirect กลับ login)
- จัดการสินค้า, คำสั่งซื้อคนอื่น, ผู้ใช้คนอื่น

---

### มุมมองที่ 2: ผู้ดูแลระบบ (Admin)

#### ขั้นตอนเข้าสู่ระบบ

มี 2 วิธีให้เลือก

**วิธี A — เข้าตรงผ่าน URL ของ Admin**
- XAMPP: http://localhost/gadgetzone/admin/
- CAMPP: http://127.0.0.1:8080/gadgetzone/admin/

หน้าจะ redirect ไปหน้า Login → กรอก:
- Email: `admin@gadgetzone.com`
- Password: `Admin@1234`

ระบบจะส่งคุณเข้า Admin Dashboard ทันที

**วิธี B — เข้าผ่านหน้าร้านก่อน**

1. เข้าหน้า Home → คลิกไอคอน **บุคคล (👤)** → Login ด้วยบัญชี admin
2. หลังล็อกอิน navbar จะแสดงไอคอน **🛠️** (admin) ขวาบนเพิ่มขึ้นมา
3. คลิกไอคอนนั้น → เข้า Admin Dashboard

#### สิ่งที่ Admin ทำได้

| เมนู | ฟีเจอร์ |
|------|----------|
| **Dashboard** | ดูภาพรวมรายได้ จำนวนคำสั่งซื้อ สินค้าใกล้หมด สินค้าขายดี Top 5 |
| **Products** | เพิ่ม / แก้ไข / ลบสินค้า, อัปโหลดรูป, กรองตามหมวดหมู่ |
| **Orders** | ดูคำสั่งซื้อทั้งหมด, ค้นหา, เปลี่ยนสถานะ (pending → processing → shipped → delivered) |
| **Users** | ดูสมาชิกทั้งหมด, เพิ่มสมาชิกใหม่, **เปลี่ยน role** (super_admin เท่านั้น), **ลบผู้ใช้** (super_admin เท่านั้น) |
| **Settings** | เลือกสกุลเงินจาก 12 สกุล, ใส่ Stripe API key เพื่อเปิดรับชำระบัตรเครดิต |

#### สิทธิ์ของแต่ละ role

| Role | เข้า admin ได้ | จัดการสินค้า / คำสั่งซื้อ | เปลี่ยน role / ลบผู้ใช้ |
|------|---------------|----------------------------|---------------------------|
| `member` | ❌ | ❌ | ❌ |
| `admin` | ✅ | ✅ | ❌ |
| `super_admin` | ✅ | ✅ | ✅ |

> บัญชี `admin@gadgetzone.com` ที่ให้มาเป็น **super_admin** จึงทำได้ทุกอย่าง
